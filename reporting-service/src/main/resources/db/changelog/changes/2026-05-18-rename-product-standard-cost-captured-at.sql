...
--comment: reporting.product_standard_cost.captured_at → updated_at. Two reasons. (1) Aligns with the dominant projection-table convention (matches finance.product_accounting.updated_at, sales.product_card.updated_at, etc.). (2) The table already has trigger trg_reporting_product_standard_cost_updated_at attached, which calls shared.set_updated_at() — that function writes NEW.updated_at, so the trigger was silently broken whenever the projection's ON CONFLICT DO UPDATE path fired (column "updated_at" did not exist). The rename makes the trigger functional and removes the need for the projection's SQL to set captured_at = now() explicitly.
DO $$
BEGIN
    IF EXISTS (
