...
| File | Role |
|---|---|
| `InvariantVerifier.java` | The shared assertion core — plain cross-schema JDBC. No oversell, double-entry balance, saga convergence (polled to a deadline, since payment → completed is async). Reused by every execution and the demo finale. Runnable standalone (`main`). |
