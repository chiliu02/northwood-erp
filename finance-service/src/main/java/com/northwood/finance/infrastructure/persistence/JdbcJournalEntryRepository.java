...
public class JdbcJournalEntryRepository implements JournalEntryRepository {
    ...
    private static final RowMapper<JournalEntry> HEADER_MAPPER = (rs, n) -> {
        ...
        return JournalEntry.reconstitute(
            ...
            JournalEntry.SourceModule.fromCode(rs.getString("source_module")),
            JournalEntry.SourceDocumentType.fromCode(rs.getString("source_document_type")),
            rs.getObject("source_document_id", UUID.class),
            rs.getString("description"),
            JournalEntry.Status.fromCode(rs.getString("status")),
    ...
    public void save(JournalEntry entry) {
        ...
        jdbc.update("""
            ...
            entry.sourceModule().code(), entry.sourceDocumentType().code(), entry.sourceDocumentId(),
            entry.description(),
