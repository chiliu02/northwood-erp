...
Wipe the docker-compose volumes and bring everything back up cleanly. Used after any change to `db/northwood_erp.sql`, when seed data needs resetting, or when Liquibase bookkeeping in the `databasechangelog` table has drifted from what's on disk.
...
- Postgres re-runs `db/northwood_erp.sql` on first boot of the fresh volume (mounted into `/docker-entrypoint-initdb.d/`) — every schema, role, grant, and seed row.
