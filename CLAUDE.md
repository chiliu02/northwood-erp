...
**`*Repository` is reserved for DDD aggregate roots.** Every `*Repository` interface in `domain/` must have a sibling aggregate root file in the same package that declares `public static final String AGGREGATE_TYPE`. A row-level write port without a backing aggregate is a code-review fail — either promote the aggregate (intent-named mutators + `pendingEvents` drained by `save()`), or pick a different suffix. Known exception today: `manufacturing.BomEditRepository`, tracked for promotion in `dev-todo.md` §2.16. Detail + rationale: `docs/conventions.md` → *No `*Repository` without an aggregate*.
