...
- **`docs/messaging.md`** — Outbox → Kafka → inbox flow, partition-key choice (`aggregateId` — the load-bearing design decision), ordering guarantees, DLT pinning, the **consumer error handler** (classify poison vs transient + `ExponentialBackOff` — `inboxErrorHandler`) and **per-service DLT auto-redrive** (`DltRedriver`, §2.28 Tier 1), audit items for scaling past 1 partition, and **consumer-side idempotency** (the `AbstractInboxHandler` `@Transactional` check → apply → record flow, the rebalance-window TOCTOU race + why the `inbox_message` unique constraint doesn't close it, and the config-selectable dedup gate — `northwood.inbox.dedup-strategy`: advisory-lock default = `pg_advisory_xact_lock`; unique-claim = portable, deferred). Load this when touching `KafkaEventPublisher`, `OutboxDrainer`, `KafkaMessagingAutoConfiguration` / `DltRedriver` (consumer error handling + DLT redrive), `SagaManager` concurrency, any topic-config change (§2.14), or `AbstractInboxHandler` / `InboxPort` / `InboxDedupStrategy` / `JdbcInboxAdapter`.
- **`docs/observability.md`** — The §1D LGTM tier (Loki/Grafana/Tempo/Prometheus + Promtail) as docker-compose sidecars: how instrumentation is wired (`shared/pom.xml` deps incl. the load-bearing `spring-boot-starter-opentelemetry`, the `management.*` `application.yml` block, the `logback-spring.xml` console+Loki appenders), the three telemetry env vars (`OTLP_ENDPOINT` / `LOKI_URL` / `NORTHWOOD_TRACING_SAMPLING`), how to run + open Grafana, ready-to-paste PromQL/TraceQL/LogQL + the trace↔log↔metric click-through, a worked Sales-Order-fulfilment example (one trace/log-stream/metrics across sales→inventory→finance), and the AWS observability-box wiring. Load this when touching any `management.*` config, the LGTM compose services / `db/{prometheus,tempo,loki,grafana}` configs, or the `enable_observability` Terraform tier.
