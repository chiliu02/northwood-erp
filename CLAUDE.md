...
- **`docs/messaging.md`** — Outbox → Kafka → inbox flow, partition-key choice (`aggregateId` — the load-bearing design decision), ordering guarantees, DLT pinning, audit items for scaling past 1 partition. Load this when touching `KafkaEventPublisher`, `OutboxDrainer`, `SagaManager` concurrency, or any topic-config change (§2.14).
