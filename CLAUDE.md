...
- **`docs/messaging.md`** — Outbox → Kafka → inbox flow, partition-key choice (`aggregateId` — the load-bearing design decision), ordering guarantees, DLT pinning, audit items for scaling past 1 partition, and **consumer-side idempotency** (the `AbstractInboxHandler` `@Transactional` check → apply → record flow, the rebalance-window TOCTOU race + why the `inbox_message` unique constraint doesn't close it, and the config-selectable dedup gate — `northwood.inbox.dedup-strategy`: advisory-lock default = `pg_advisory_xact_lock`; unique-claim = portable, deferred). Load this when touching `KafkaEventPublisher`, `OutboxDrainer`, `SagaManager` concurrency, any topic-config change (§2.14), or `AbstractInboxHandler` / `InboxPort` / `InboxDedupStrategy` / `JdbcInboxAdapter`.
