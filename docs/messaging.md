...
### Event disorder: where it enters, how it's absorbed, the three legal buckets

Step back from the per-saga view to the system-wide one. **Disorder can enter at four layers, each with a different guarantee:**
...
**The design principle.** In a distributed system, global total order isn't worth buying — the mechanisms that enforce it (single partition, a global sequencer, distributed locks) trade away the throughput and availability the event architecture exists to gain. So the goal is not to *avoid* disorder but to **minimise the disorder that actually matters**, which is the intersection of two sets: orderings that **can occur** and orderings your **correctness depends on**. A reordering only bites when it sits in both. You attack it from both sides: **buy the cheap local ordering guarantees first** (partition-key = `aggregateId` for per-aggregate order; causal-by-construction emission — commit the prerequisite before publishing the dependent), **then apply the minimum tolerance machinery to whatever residual remains** (idempotent / convergent / state-guarded handlers + inbox dedup). Tolerance is not free either, so over-tolerating — making everything order-independent when a cheap causal guarantee was available — is its own over-engineering.

Every ordering assumption must therefore land in exactly **one of three legal buckets**:

- **guaranteed** — same partition (per-aggregate key) or causal emission (prerequisite committed upstream before the dependent is emitted);
- **guarded** — a source-state or terminal guard makes a late/early event no-op instead of mis-applying;
- genuinely **don't-care** — the operation is commutative / convergent (atomic increment, idempotent upsert), so arrival order is immaterial.

The bug class is the **illegal fourth bucket** — an *implicit* assumption that A precedes B, neither guaranteed nor guarded; that is precisely what the PR→PO residual (below) was. Much of the tolerance machinery is owed to at-least-once delivery anyway (redelivery forces idempotency regardless), so building for exactly-once-effect and building for disorder-tolerance largely pay the same bill once.
...
- **Drain by status flag, not a high-water cursor.** `findPending` is `SELECT … WHERE status IN ('pending','failed') ORDER BY sequence_number … FOR UPDATE SKIP LOCKED` — it re-scans *pending* rows every tick rather than paginating past a `sequence_number > :cursor` watermark. That dodges the classic **commit-order trap**: a long transaction can grab a low `sequence_number` early but commit *after* a higher one, so a watermark consumer would skip the late-committing low row forever — whereas here that row is simply still `pending` on a later tick and gets published then. The `ORDER BY sequence_number` does the weaker job of publishing each batch in write order, which (with one publish in flight per service) means a service never emits out of its own write order. A partial index on `WHERE status IN ('pending','failed')` keeps the working index small as `published` rows accumulate. (The `last_sequence_number` column in `projection_checkpoint` is a relic of an earlier cursor-polling design intent; the live read side is Kafka offsets + inbox dedup.)
...
- **Adding an inbox handler also means adding the producer's topic to `subscribe-topics`.** A new `AbstractInboxHandler` subclass registers the *event-type → handler* route, but it does nothing until the consuming service is actually subscribed to the topic the event lands on — every producer emits to `<service>.events` (`KafkaEventPublisher.EVENTS_TOPIC_SUFFIX`), so consuming a new cross-service event requires that producer's `<service>.events` be in the consumer's `northwood.kafka.subscribe-topics`. The two edits are a pair; the handler alone is a silent no-op. **The reactor build will not catch the gap** — the acceptance `*DslTest` tier runs on a synchronous in-memory bus with no per-service subscription list, so a missing `subscribe-topics` entry is green in `mvn verify` and only fails on the live Kafka stack (the saga parks forever waiting for an ack that is published but never consumed). This is exactly the bug the live load/race tier (`load-test`) exists to surface: multi-leg saga compensation's purchasing-leg ack (`purchasing.PurchaseOrderCancellationApplied`) hung at `compensating` because sales subscribed to `manufacturing.events` (make-to-order leg worked) but not `purchasing.events` (buy-to-order leg dead). Symptom to recognise: a published producer outbox row (`status='published'`) with **no** matching consumer `inbox_message` row, and `KafkaInboxDispatcher` DEBUG logs showing the consumer only ever sees its existing topics. The dispatcher safely no-ops event types it has no handler for (`no handler for purchasing.PurchaseOrderCreated …`), so over-subscribing to a producer's whole `.events` topic to reach one event type is side-effect-free.
