...
### Rule: one concern, one consumer — don't split a single logic across consumer groups

When a single logical operation has several side-effects that must **converge** — e.g. the replenishment "mark the request `dispatched` **and** stamp its PO link" — keep them in **one** consumer. Splitting them across two consumer groups gives each its own offset, so partition ordering cannot serialise them: you get a cross-consumer race the partition key can't fix (different keys → different partitions; and even the *same* key → two consumers advance independently), **plus** a single-responsibility violation — the second concern bolted onto an unrelated handler.

The PR→PO link was the worked example. The link was attached to the line-facts seeder (`inventory.purchase-order-line-facts`) — a *different* consumer than the dispatch handler (`inventory.replenishment.pur-dispatched`) — purely as an economy, because that seeder already subscribed to `PurchaseOrderCreated`. That split is exactly what created the dispatch-vs-PO-created race. The fix put the link in the replenishment concern (`inventory.replenishment.pur-po-created`) so dispatch and link share one concern, and made the seeder do one thing again.

The escape hatch, when two side-effects genuinely must live in different consumers (they serve different read models): make each handler **idempotent and self-sufficient** — carry the proof/correlation it needs *on the event* so it can apply regardless of what the other consumer has done yet (the PR→PO fix forwards `sourceReplenishmentRequestId` and performs the dispatched-state write itself). Reach for that, **not** for re-keying the event or routing both types to one ordered consumer — those buy an infra ordering invariant you must defend forever (and, for re-keying, fight the `aggregateId`-key convention and the multi-purpose nature of the event). Idempotent self-sufficiency keeps correctness in the domain logic. The full (a)/(b)/(c) trade-off is dissected in `reverse-engineered-domain-model.md` → *the legal fixes*.
