...
| Consumer messages dead-lettered during an outage **longer than the retry budget** | 🛠 manual | after ~5 min the record is dead-lettered → same DLT-replay as above (or auto-redrive once §2.28 Tier 1.B lands). |
| Consumer **deterministic / poison** failure (bad payload, constraint, NPE) | 🛠 manual | classified non-retryable → dead-lettered on the first failure, no wasted retries (§2.28 Tier 1.A) → DLT-replay as above. |
