...
What an inbox handler does when it can't find its saga: `requireSaga` throws `IllegalStateException`. In normal operation this **never fires** — every saga is created before the request whose reply is the consequence event, so the consequence is causally downstream of the saga row and can't precede it, on any number of partitions (see *Hazards when scaling past 1 partition*, item 4). The throw is a defensive guard against a genuine fault (saga creation rolled back, row deleted): non-retryable → DLT → `DltRedriver` replay once the row is restored. So there is no "park the consequence until the saga appears" path to re-test — the ordering is structural, not timing-dependent.
