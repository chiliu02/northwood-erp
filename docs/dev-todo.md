...
|---|---|---|---|
| **1D.2** | Cross-service trace propagation | 1D.1 (shipped) | Micrometer Tracing's Kafka instrumentation auto-handles `traceparent` headers once 1D.1 lands. OTel auto-instruments Spring's `RestClient` (saga aggregator polling, ERP BFF). `OutboxPublisher` also stamps `traceparent` into `EventEnvelope.headers` so the SPA can read it without a separate JOIN. Verify: drive place-order, one trace tree spans sales → inventory → manufacturing → finance. |
