...
- **2.0.a Foundation** — `Product.Status` (refactor plain enum), `Bom.Status` (refactor plain enum) + `Bom.ComponentKind`, `WorkOrder.{Status, MaterialStatus, MaterialLineStatus, OperationStatus}`, `Customer.Status` (verify dbValue style).
