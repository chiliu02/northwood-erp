...
**2.24.3 Replace N+1 SQL walk with single recursive-CTE query in `BomLookup` ✅ shipped 2026-05-18.** New `BomLookup.findActiveBomTreeRows(rootProductId)` returns the entire hierarchy in one recursive CTE with cumulative-quantity multiplication computed in SQL. Both `BomViewService` methods now make exactly one DB read. See `dev-done.md`.
