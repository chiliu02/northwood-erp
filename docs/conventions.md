...
- **Tables named after a facet group** (e.g. `sales.product_pricing`, `finance.product_accounting`, `manufacturing.product_replenishment`) — these were the previous-generation pattern; rename to `_card` once consolidated. Migration tracked in `dev-todo.md` §2.23.
