...
- **Tables named after a facet group** (e.g. `manufacturing.product_replenishment`) — these were the previous-generation pattern; rename to `_card` once consolidated. Migration tracked in `dev-todo.md` §2.23.
...
| **Snapshot projection of upstream state** — local cache of a fact owned by another service | `sales.product_card`, `manufacturing.product_active_bom`, `manufacturing.product_replenishment`, `purchasing.product_approved_vendor`, `finance.product_card`, all 6 reporting views | `*Projection` (write) + `*Lookup` / `*QueryPort` (read) | The mutation authority lives upstream with the producer. The consumer reads locally because per-service `search_path` blocks cross-schema joins, but the consuming service holds no invariants over the data. |
