...
**Known exceptions today** (three legacy offenders surfaced by the 2026-05-15 / 2026-05-16 audit; new code must not add more):
...
- `purchasing.domain.SupplierProductPriceRepository` — no `SupplierProductPrice` Java class; row-level write port for `purchasing.supplier_product_price`. `SupplierProductPriceChanged.AGGREGATE_TYPE` lives on the event class as the cross-service-emission exception. Promotion tracked in `dev-todo.md` §2.17.
- `product.domain.ApprovedVendorRepository` — `ApprovedVendor` is a value object of `Product`, not an aggregate root. The repository exists because `product.approved_vendor` is denormalised out into its own table. Resolution tracked in `dev-todo.md` §2.17.

Two prior offenders surfaced by the same audit were resolved in §2.18: `manufacturing.RoutingRepository` and `purchasing.SupplierRepository` had no mutators / events / `pendingEvents` — they were renamed to `RoutingQueryPort` / `SupplierQueryPort` (with the interfaces moved from `domain/` to `application/`) since they're CQRS read-side ports, not DDD repositories.
