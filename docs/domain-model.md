...
**Saga.** `SalesOrderFulfilmentSaga` is a **pruned orchestration**: its states are only the points it actually branches on, not a mirror of the order's status. States: `started` → `awaiting_release` (planning-time-fence park, woken by the poll) / `awaiting_prepayment` → `prepaid` (up-front-payment gate) → `stock_reservation_requested` → `stock_reservation_incomplete` (short — wait for `ReplenishmentFulfilled`) → `supply_secured` (completion gate: accumulate `orderShipped` + `orderSettled` latches) → `completed`; plus `rejected` (unsourceable), `compensated` (cancel path — entered directly from the active state on inventory's cancellation ack; the two-phase cancel has no intermediate `compensating` state — it was removed), `failed`. The post-supply ship → invoice → pay status is the line fold + the 360, so those pass-through states were dropped.
