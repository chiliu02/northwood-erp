...
**Application services.** `SalesOrderService` — `placeOrder`, `addLine`, `changeLineQuantity`, `changeLineUnitPrice`, `removeLine`, `cancel` (phase 1 — *requests* cancellation), `confirmCancellation` (phase 2 — finalises on inventory's applied-ack), `recordShipped`. `CustomerService` — `registerCustomer`, `changeName`, `changeContact`, `changeBillingAddress`, `changeShippingAddress`, `deactivate`.
...
**Saga.** `SalesOrderFulfilmentSaga` is a **pruned orchestration**: its states are only the points it actually branches on, not a mirror of the order's status. States: `started` → `awaiting_release` (planning-time-fence park, woken by the poll) / `awaiting_prepayment` → `prepaid` (up-front-payment gate) → `stock_reservation_requested` → `stock_reservation_incomplete` (short — wait for `ReplenishmentFulfilled`) → `supply_secured` (completion gate: accumulate `orderShipped` + `orderSettled` latches) → `completed`; plus `rejected` (unsourceable), `compensated` (cancel path — entered directly from the active state on inventory's cancellation ack; the older `compensating` hop is now bypassed, since the two-phase cancel no longer pre-compensates), `failed`. The post-supply ship → invoice → pay status is the line fold + the 360, so those pass-through states were dropped.
