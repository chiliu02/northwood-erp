...
The narrowest-possible-projection rule still applies: `inventory.sales_order_line_facts` carries only what inventory's write-path guards need — `(line_id, header_id, product_id)` for the shipment-line product check, `payment_terms` / `upfront_settled` for the prepayment gate, and `ordered_quantity` / `shipped_quantity` / `cancelled` for the synchronous ship-claim + cancel-vs-ship arbiter (`docs/validations.md` → *Concurrency under contention*) — **not** a 1:1 mirror of `sales.sales_order_line`. The consumer projects only what it needs to validate and arbitrate, no more.
