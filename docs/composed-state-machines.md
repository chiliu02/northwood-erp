...
(The `?` marks `WAITING_FOR_PRODUCTION` / `READY_TO_SHIP`. These are **model-only** bands —
they were removed from `SalesOrder.LineStatus` + the schema CHECK on 2026-06-10 because a
to-order line's production progress is tracked off the line, §11.4; the live line chain is
...
- ✅ **To-order is built** (`product.ReplenishmentStrategy.TO_ORDER`; make-to-order +
  buy-to-order E2E) — but it adds **no** line-status bands. A to-order line's production/receipt
  progress lives off the line (saga + work order + reporting's `manufacturing_status` /
  `stock_status`); the SO line runs the one `open → reserved → shipped` chain like a stock line.
  The model-only `WAITING_FOR_PRODUCTION` / `READY_TO_SHIP` values were removed from
  `SalesOrder.LineStatus` + the schema CHECK (2026-06-10), §11.4.
...
only *that* it does. `WAITING_FOR_PRODUCTION` (a model-only band — not a `SalesOrder.LineStatus`
value; see §11.4) would be simply a refinement of the `IN_PROGRESS` band that only make lines
visit. The fold runs on
...
The opt-in **to-order extension** (`product.ReplenishmentStrategy.TO_ORDER`) is in fact
**built** — `OrderToCashMakeToOrderPathTest` / `OrderToCashBuyToOrderPathTest` exercise a
sales-order line that waits on a work order or a purchase. But the prediction this section
originally made — that a to-order line would carry `WAITING_FOR_PRODUCTION` / `READY_TO_SHIP`
*line-status* bands — **did not happen, and the design is better for it.** A to-order line's
production/receipt progress lives **off the line** (the fulfilment saga state + the
manufacturing work order / inventory replenishment + reporting's `manufacturing_status` /
`stock_status`); the SO line itself runs the *one* `OPEN → RESERVED → SHIPPED` chain — identical
to a stock line — once its order-pegged supply is reserved. So §11.1's result holds even more
strongly than predicted: not only does the fold not branch on product type, the line doesn't
even need variant-specific states. The `WAITING_FOR_PRODUCTION` / `READY_TO_SHIP` enum values
were accordingly **removed** from `SalesOrder.LineStatus` + the schema CHECK (2026-06-10) — they
are model-only bands (§2.1/§13), never produced. The stratified §11.3 product would only be
needed if to-order required per-kind status *reporting* on the order header, which it does not.
...
for the **full model chain** — *not* limited to what Java emits. The make/buy concrete
line-states below (`waiting_for_production` / `ready_to_ship` / `awaiting_receipt`) are
**illustrative model bands, not `SalesOrder.LineStatus` values**: to-order is built but the SO
line never carries them (production progress lives off the line, §11.4), so the live line emits
only `open` / `reserved` / `partially_reserved` / `partially_shipped` / `shipped` / `cancelled`.
Three worked examples covering every case in this note follow.
...
- **Per-kind status *reporting* on the header (§11.3)** — to-order itself is built, but the
  ...
  needed and not done. The SO line runs the one chain regardless of variant, so the
