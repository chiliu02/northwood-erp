...
- `applyInventoryCancellationApplied(SO, legIds)` — inventory's reservation-release ack + the second phase of the two-phase cancel. The handler first calls `SalesOrderService.confirmCancellation` (flips the header to `cancelled`), then: **empty `legIds`** (the common path — nothing order-pegged was committed) → advances any non-terminal saga straight to `compensated` and emits `SalesOrderCompensated`; **non-empty `legIds`** → stamps them on `FulfilmentSagaData.outstandingCompensationLegs` and parks the saga in `compensating`. (A terminal saga — the `rejected`/unsourceable path, which also releases via this ack — is left untouched.)
- `applyCompensationAck(SO, legId, failed)` — the **multi-leg compensation drain** (a third instance of the saga's set-drain join, alongside `outstandingReplenishmentLineIds` and the completion gate). Each `purchasing.PurchaseOrderCancellationApplied` / `manufacturing.WorkOrderCancellationApplied` drains its leg (`<targetService>:<salesOrderLineId>`); idempotent on an already-drained leg. When the outstanding set empties the saga branches: no failures → `compensated`; any failure (an un-compensatable leaf — PO already received / WO in progress refused) → the `compensation_failed` terminal, emitting `SalesOrderCompensationFailed` to escalate. **`compensation_failed` ≠ `failed`:** the former is a clean business outcome (order cancelled, a residue needs a human) the latter a broken saga. Topology: inventory fans out the per-leg `OrderPeggedSupplyCancellationRequested` because it owns the peg→PO/WO map; the sales saga is the orchestrator that counts the acks. Full design: `docs/compensation.md`.
