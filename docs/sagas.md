...
Sagas that own their own emissions independently of any domain aggregate carry the constant on the saga state-machine class (`SalesOrderFulfilmentSaga.AGGREGATE_TYPE`), which in turn re-exports from `SalesAggregateTypes.SALES_ORDER_FULFILMENT_SAGA`.
