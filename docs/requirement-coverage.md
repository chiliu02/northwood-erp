...
| REQ-SAL-036/040 (cancel / compensation) | `OrderToCashCancellationPathDslTest`, `OrderToCashCancelRefundPathDslTest` (release reservations + refund) | ✅ |
| REQ-SAL-061 (multi-leg compensation — withdraw committed order-pegged PO/WO) | `OrderToCashCompensatePurchasedLegDslTest` (PO leg), `OrderToCashCompensateManufacturedLegDslTest` (WO leg), `OrderToCashCompensatePrepaidToOrderLegDslTest` (deposit + leg + refund); domain refusal (`PurchaseOrderTest.CompensateCancel`, `WorkOrderTest.Cancel`) + saga drain (`JdbcSalesOrderFulfilmentSagaManagerTest.ApplyCompensationAck` → `compensated`/`compensation_failed`) + sub-saga termination (`JdbcPurchaseToPaySagaManagerTest`, `JdbcWorkOrderSagaManagerTest` cancel) | ✅ |
| REQ-SAL-041 (hard cancel during mfg) | `OrderToCashCompensateManufacturedLegDslTest` (released-WO withdrawal) + `WorkOrderTest.Cancel` (in-progress refuses) | ⚠️ partial — released-WO compensation shipped; operator hard/soft-cancel of an *in-progress* WO still deferred (see REQ-MFG-060) |
...
| REQ-MFG-060 (WO cancel) | `WorkOrderTest.Cancel` + `JdbcWorkOrderSagaManagerTest.Cancel` + `OrderToCashCompensateManufacturedLegDslTest` (compensation path) | ⚠️ partial — `WorkOrder.cancel()` shipped on the **compensation** path (released-WO withdrawal, REQ-SAL-061); the **operator-facing** cancel endpoint + in-progress halt/write-off still deferred |
...
| REQ-FIN-033 (deposit refund on cancel/reject, on the confirmed terminal) | `CustomerCancellationRefundServiceTest` (6 cases: paid deposit/prepayment refund Dr 2110/Cr 1000; commercial/no-invoice/unpaid/already-refunded no-op); `OrderToCashCancelRefundPathDslTest` + `OrderToCashCompensatePrepaidToOrderLegDslTest` (e2e refund on `SalesOrderCompensated`, 2110 nets to zero); `OrderToCashRejectedPathDslTest` (`SalesOrderRejected` emitted) | ✅ |
