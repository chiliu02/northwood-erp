...
Each entry follows this shape, with a delivery-status icon directly after the code — **✅ shipped · ⚠️ partially shipped · ❌ not shipped** (planned / deferred):
...
**REQ-PROD-060 ⚠️ — Each makeable SKU may have one active Bill of Materials** *(shipped — read-only authoring)*
...
**REQ-MFG-002 ⚠️ — BOM authoring** *(partial)*
...
**REQ-MFG-010 ⚠️ — Each makeable SKU may have an active routing** *(shipped — read-only)*
...
**REQ-FIN-061 ❌ — Cross-currency conversion via exchange-rate table** *(deferred)*
**Multi-currency is not implemented** — the system runs single-currency (AUD) only (REQ-FIN-060 is the live path). The intended design — a rate-table lookup with inverse-rate fallback for missing direct rates (triangulation through a base currency out of scope) — is not delivered: the `CurrencyConverter` / exchange-rate scaffolding exists but is not wired into the posting paths. Deferred (low priority).

**REQ-FIN-062 ❌ — Ad-hoc rate lookup** *(deferred)*
Part of the unbuilt multi-currency feature (REQ-FIN-061). The `GET /api/exchange-rate?from=&to=&date=` endpoint exists as scaffolding, but multi-currency conversion is not delivered (single-currency only). Deferred (low priority).
