...
/**
 * In-memory {@link WorkOrderSagaPort}. Same single-threaded harness
 ...
 * version.
 */
public final class InMemoryWorkOrderSagaPort implements WorkOrderSagaPort {

    private final Map<UUID, WorkOrderSaga> bySagaId = new HashMap<>();

    @Override
    public List<WorkOrderSaga> claimDue(int batchSize, Set<String> activeStates,
                                          String leaseOwner, Duration leaseTtl) {
        Instant now = Instant.now();
        List<WorkOrderSaga> due = new ArrayList<>();
        for (WorkOrderSaga s : bySagaId.values()) {
