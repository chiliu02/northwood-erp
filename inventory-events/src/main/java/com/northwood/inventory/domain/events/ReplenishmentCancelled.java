...
/**
 * §2.37 Slice 2: a replenishment request was cancelled before it could fulfil
 * — the downstream service couldn't source it (manufacturing has no active BOM,
 * purchasing has no supplier / the SKU is discontinued) or inventory found the
 * SKU unsourceable. Emitted by {@code ReplenishmentRequest.markCancelled}.
 *
 ...
 * fan-in (§2.37 Slice 3) can route the cancellation to the right saga and flip
 * the order to {@code rejected} — without a join back to
