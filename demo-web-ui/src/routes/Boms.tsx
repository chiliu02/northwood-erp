...
import { fetchProducts, fetchBomTree, NotFoundError } from "@/api/fetchers";
import type { BomNode, BomTree, ProductRow } from "@/api/types";
...
export function Boms() {
  ...
  const notFound = treeError instanceof NotFoundError;
    ...
    <div className="space-y-4">
      ...
      {!productId ? (
        ...
        >
          ...
          <p className="mt-1 text-text-muted">{String(treeError)}</p>
        </div>
      ) : tree ? (
